...
class LocalDatabase {
  ...
  Future<int> insertSyncOperation(SyncOperation operation) async {
    ...
    return await db.insert('sync_operations', {
      ...
      'metadata': operation.metadata != null ? operation.metadata.toString() : null,
  ...
  Future<List<SyncOperation>> getPendingSyncOperations() async {
    ...
    return List.generate(maps.length, (i) {
      final dataStr = maps[i]['data'] as String;
      final data = _parseJsonString(dataStr);
      
      final metadataStr = maps[i]['metadata'] as String?;
      final metadata = metadataStr != null ? _parseJsonString(metadataStr) : null;
  ...
  Map<String, dynamic> _parseJsonString(String jsonString) {
    // Remove outer braces if present and convert to proper JSON
    ...
    try {
      // Simple parsing of key-value pairs if it's not valid JSON
      if (!cleanString.contains('{') && !cleanString.contains('}')) {
        ...
        return Map<String, dynamic>.from(json.decode(cleanString));
      }
    } catch (e) {
